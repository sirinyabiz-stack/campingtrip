...
const DEMO_DIRECT_EXPENSES = [
  {
    ...
    type: 'calculated', // calculated หรือ flat
    peopleCount: 4,
    nightCount: 3,
...
const DEMO_FOOD_ITEMS = [
  { id: 'food-1', title: 'เนื้อสไลด์สำหรับปิ้งย่างหมูกระทะ', quantity: 2, unitPrice: 280, completed: true },
  { id: 'food-2', title: 'เซ็ตผักสดรวมและบะหมี่กึ่งสำเร็จรูป', quantity: 1, unitPrice: 150, completed: true },
  { id: 'food-3', title: 'น้ำดื่มและถังน้ำแข็ง', quantity: 3, unitPrice: 65, completed: false }
];

export default function App() {
  // === STATE ===
  const [activeTab, setActiveTab] = useState('dashboard'); // dashboard, trips, expenses, food, stats, settings
  ...
  const [statusFilter, setStatusFilter] = useState('ALL'); // ALL, PLANNING, UPCOMING, COMPLETED
  ...
  const [foodForm, setFoodForm] = useState({ title: '', quantity: 1, unitPrice: '' });
  ...
  const averageTripCost = trips.length > 0 ? (totalDirectAll + totalSharedAll) / trips.length : 0;
  ...
  const handleSaveFoodItem = (e) => {
    ...
    if (editingFoodId) {
      setFoodItems(foodItems.map(f => f.id === editingFoodId ? { ...f, ...parsedItem } : f));
      setEditingFoodId(null);
      triggerToast('แก้ไขรายการอาหารสำเร็จ');
    } else {
      const newItem = { ...parsedItem, id: 'food-' + Date.now() };
      setFoodItems([...foodItems, newItem]);
      triggerToast('เพิ่มรายการเตรียมอาหารใหม่เรียบร้อย');
    }

    setFoodForm({ title: '', quantity: 1, unitPrice: '' });
  };
  ...
  const handleExportCSV = () => {
    let csvContent = "data:text/csv;charset=utf-8,\uFEFF"; // Thai UTF-8 BOM
    ...
    // Food
    csvContent += "\n=== รายการเตรียมอาหาร ===\n";
    csvContent += "รายการ,จำนวน,ราคาต่อชิ้น,รวมราคา,สถานะจัดเตรียม\n";
    foodItems.forEach(f => {
      csvContent += `"${f.title}",${f.quantity},${f.unitPrice},${f.quantity * f.unitPrice},"${f.completed ? 'จัดเตรียมแล้ว' : 'ยังไม่ได้เตรียม'}"\n`;
    });
      ...
      <main className="max-w-4xl mx-auto px-4 pt-5">
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-5">
            ...
            <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-emerald-950 via-teal-900 to-stone-900 text-white p-5 shadow-xl">
              <div className="relative z-10 space-y-2">
                <div className="inline-block bg-emerald-500/20 border border-emerald-400/30 text-emerald-300 text-[10px] px-2.5 py-0.5 rounded-full font-extrabold uppercase">
                  Let's explore mountains & forests 🏔️
                </div>
            ...
            {/* Recent Trips and Quick Tasks Grid */}
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-right shrink-0">
                            ...
                            <p className="text-xs font-extrabold text-amber-500">฿{(totalDirect + totalShared).toLocaleString(undefined, {maximumFractionDigits: 0})}</p>
                          </div>
                        </div>
                      );
                    })}
        ...
        {/* 2. TRIPS TAB (WITH TRIP STATUS FILTER) */}
        {activeTab === 'trips' && (
          <div className="space-y-5">
            ...
            <div className={`p-4 rounded-xl border space-y-3.5 ${darkMode ? 'bg-zinc-900 border-zinc-850' : 'bg-white border-stone-200'}`}>
              ...
              {/* Advance province & month dropdown filter */}
              <div className="grid grid-cols-2 gap-2 border-t pt-3">
                <select 
                  value={provinceFilter}
                  onChange={(e) => setProvinceFilter(e.target.value)}
                  className={`text-[11px] py-1.5 px-3 rounded-lg border outline-none cursor-pointer ${darkMode ? 'bg-zinc-800 border-zinc-750 text-white' : 'bg-stone-50 border-stone-200'}`}
                >
            ...
            {filteredTrips.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredTrips.map(trip => {
                  ...
                  const total = directVal + sharedVal;
                  const statusConf = TRIP_STATUSES[trip.status];
        ...
        {activeTab === 'food' && (
          <div className="space-y-5 animate-fade-in">
            <div>
              ...
              <p className="text-xs text-zinc-400">ลงรายการของกิน น้ำดื่ม และวัตถุดิบปรุงอาหารสำหรับเช็คก่อนออกเดินทาง</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              
              <div className={`p-4 rounded-xl border h-fit ${darkMode ? 'bg-zinc-900 border-zinc-850' : 'bg-white border-stone-200'}`}>
                <h3 className="font-bold text-xs text-zinc-400 mb-3 uppercase">ระบุของกินเพิ่มเติม</h3>
                ...
                <form onSubmit={handleSaveFoodItem} className="space-y-3.5">
                  ...
                  <div className="flex gap-2">
                    <button type="submit" className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold py-2 rounded-lg">
                      {editingFoodId ? 'อัปเดต' : 'เพิ่มรายการ'}
                    </button>
                    {editingFoodId && (
                      <button 
                        type="button" 
                        onClick={() => { setEditingFoodId(null); setFoodForm({ title: '', quantity: 1, unitPrice: '' }); }}
              ...
              <div className={`md:col-span-2 p-4 rounded-xl border ${darkMode ? 'bg-zinc-900 border-zinc-850' : 'bg-white border-stone-200'}`}>
                <div className="flex justify-between items-center mb-3">
                  <h3 className="font-bold text-xs text-zinc-400 uppercase">ลิสต์ตรวจทานสเบียงของทริป</h3>
                  <span className="text-xs font-black text-emerald-500">รวม ฿{totalFoodCost.toLocaleString()}</span>
                </div>
                    ...
                    return (
                      <div key={trip.id} className="border-b dark:border-zinc-850 last:border-0 pb-3 last:pb-0 text-xs">
                        ...
                        <div className="flex justify-between items-center text-[10px] text-zinc-400 mt-1">
                          ...
                          <span>(เฉพาะทริป: ฿{direct.toLocaleString()} | ค่าแชร์เฉลี่ย: ฿{shared.toLocaleString()})</span>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className={`p-4 rounded-xl border ${darkMode ? 'bg-zinc-900 border-zinc-850' : 'bg-white border-stone-200'}`}>
                ...
                <div className="space-y-2">
                  ...
                  <button 
                    ...
                    className="w-full bg-stone-100 dark:bg-zinc-800 hover:bg-stone-200 dark:hover:bg-zinc-750 text-zinc-700 dark:text-zinc-200 font-semibold text-xs py-2.5 rounded-lg flex items-center justify-center gap-1.5"
                  >
                    สั่งพิมพ์ข้อมูล PDF
                  </button>
                </div>
